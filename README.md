...
13. [Inference Colab/Kaggle](#13-inference-colabkaggle)
...
├── DockBench_Inference.ipynb  # Colab/Kaggle: molgrid + score 3ERK
...
## 13. Inference Colab/Kaggle

File [`DockBench_Inference.ipynb`](DockBench_Inference.ipynb): clone repo GitHub → load `best_model.pt` → score **3ERK**.

```text
https://github.com/nWoWolfpac/KhoaLuanTotNghiep.git
```

1. Mở notebook trên Colab/Kaggle (Runtime → GPU)
2. Chạy ô clone (mặc định pull `code_docking/` từ repo trên)
3. `git lfs pull` nếu checkpoint `.pt` dùng LFS
4. Thêm `tools/gninatyper` (Linux) vào repo hoặc build theo notebook
