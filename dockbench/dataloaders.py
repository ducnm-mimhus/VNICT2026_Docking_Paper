...
class GriddedExamplesLoader:
    ...
    rmsd_pos: Optional[int]
        Optional RMSD (Å) label index for pose relabeling / focal ignore band
    ...
    def __init__(
        ...
        self.rmsd_pos = rmsd_pos
        if self.rmsd_pos is not None and self.affinity_pos is None:
            raise ValueError("rmsd_pos requires affinity_pos (same batch layout).")
    ...
    def __next__(self):
        ...
        if not self.grids_only:
            ...
            if self.rmsd_pos is not None:
                rmsd_vals = torch.zeros((batch_size,), device=self.device)
                batch.extract_label(self.rmsd_pos, rmsd_vals)
        ...
        if self.grids_only:
            ...
            if self.rmsd_pos is not None:
                return grids, labels, affinities, rmsd_vals
